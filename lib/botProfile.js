...
export function getBotProfile(botUserId) {
  ...
  // Find bot by ID and isAIBot flag, or by ownerId for AI chat bot
  const botUser = allUsers.find(u => 
    u.id === botUserId && (u.isAIBot || u.ownerId) // Support both regular bots and AI chat bot
  );
...
export function updateBotProfile(botUserId, profileUpdates) {
  ...
  try {
    const allUsers = getAllUsers();
    // Find bot by ID and isAIBot flag, or by ownerId for AI chat bot
    const botIndex = allUsers.findIndex(u => 
      u.id === botUserId && (u.isAIBot || u.ownerId) // Support both regular bots and AI chat bot
    );
