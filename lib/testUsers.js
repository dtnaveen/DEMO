...
export function setupTestUsers() {
  ...
  try {
    ...
    let freeUser = allUsers.find(u => 
      u.email === credentials.free.email || u.name?.toLowerCase() === 'free user'
    );
    ...
    let premiumUser = allUsers.find(u => 
      u.email === credentials.premium.email || u.name?.toLowerCase() === 'premium user'
    );
    ...
    console.log(`- Free User: ${credentials.free.email} / [From .env.local]`);
    console.log(`- Premium User: ${credentials.premium.email} / [From .env.local]`);
...
export function loginAsFreeUser() {
  ...
  try {
    const allUsers = getAllUsers();
    const credentials = getTestCredentials();
    const freeUser = allUsers.find(u => u.email === credentials.free.email);
...
export function loginAsPremiumUser() {
  ...
  try {
    const allUsers = getAllUsers();
    const credentials = getTestCredentials();
    const premiumUser = allUsers.find(u => u.email === credentials.premium.email);
